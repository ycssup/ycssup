# 👋 Hi

📊 Investment Risk Management → Data-driven Risk & Analytics  
🎓 Full-time MBA @ CUHK (2026–2027)  
🌍 US → Shanghai → Hong Kong

---

## 🧭 About Me

Risk management professional with experience across insurance, consulting, trust, and wealth management institutions.
- 7+ years in **multi-asset portfolio risk management**
- Focus on **risk governance, risk monitoring, fund due diligence, and regulatory frameworks**
- Currently building **data-driven risk analytics tools using Python**
- Long-term focus: **Risk + Data + Strategy integration**

---

## 🚀 Featured Projects

### 🔹 Fund Risk Analytics (In Progress)
- **Problem**: Monitor portfolio risk dynamically  
- **Approach**:
  - Risk-Adjusted Return
  - Return Metrics
  - Risk Metrics
  - Drawdown Analysis
- **Goal**: Build a lightweight risk dashboard  
- **Tech**: Python, pandas, numpy, matplotlib  

---

### 🔹 Factor-Based Attribution (Planned)
- **Problem**: Decompose portfolio returns into risk factors  
- **Approach**:
  - Barra-style factor model (simplified)  
  - Style & factor exposure analysis  
- **Focus**: Equity long-only strategies  

---

## 🧠 Skills

### 📊 Risk & Finance
- Portfolio Risk Monitoring  
- Multi-asset Risk management
- Fund Due Diligence  
- Risk Governance & ERM  

### 💻 Data & Technology
- Python (pandas, numpy, scikit-learn)  
- Data Analysis & Visualization  

### 🛠 Tools
- GitHub  
- VS Code | Trae
- ChatGPT | Gemini | DeepSeek | Kimi
- Codex | Copilot
- Python | Excel | PowerPoint  

---

## 📚 Education

- 🎓 MBA (Full-time), Chinese University of Hong Kong, 2026–2027

- 🎓 M.S. Quantitative Financial Economics, Oklahoma State University, 2015-2017

- 🎓 B.S. Finance & Accounting, Indiana University Bloomington, 2011-2015

---

## 🏆 Certifications & Awards

- CFA Charterholder  
- Certified FRM 
- CAIA Charterholder  
- Sustainable Investing Certificate  
- 🏅 Top Talent Scholarship (CUHK)

---

## 🌱 Current Focus

- Developing **portfolio risk analytics tools**
- Focused on integrating finance, risk management, data, and AI to support strategic decision-making and long-term value creation

---

## 📫 Contact

- 📧 Email: yingchl2011@gmail.com  
- 🔗 LinkedIn: https://linkedin.com/in/yingchl  

---

## ⚡ Philosophy

> Risk management is not only about control —  
> it is about enabling better decisions under uncertainty.
